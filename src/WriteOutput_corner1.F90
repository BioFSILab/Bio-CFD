...
1     FORMAT("out/",A3,"_butter_fielddata.",i9.9,".",i3.3,".",f7.1,".",f8.6,".",i3.3,".dat")
           OPEN(newunit=file_unit, FILE = filename1, STATUS = "unknown")
            WRITE(file_unit,*)'variables="x","y","z","u","v","w","p","totime","cellid","cell_n","cell_pr"'
            WRITE(file_unit,*) "zone, ", "i = ", blk%nx," j = ", blk%ny, " k = ", blk%nz
            ...
            DO i = 2, blk%nx+1
               ...
               WRITE(file_unit,*) blk%xp(i),blk%yp(j),blk%zp(k), u1, v1, w1, &
 ...
 22          FORMAT("out/Chkpt/",A3,"_butter_chkpt.",i3.3,".",f6.1,".",f8.6,".dat")
      ...
      END SUBROUTINE writeResult
         ...
         integer :: file_unit
          ...
          OPEN(newunit=file_unit,FILE=filename1,STATUS="unknown")
          WRITE(file_unit,*) 'TITLE = "FEstressplot"'
          WRITE(file_unit,*) 'VARIABLES= "x", "y", "z","bd_n"'
          WRITE(file_unit,*) "ZONE NODES= ",blk%ibNodes,",ELEMENTS=",blk%ibElems,",DATAPACKING=POINT, ZONETYPE=FETRIANGLE"
          DO inode = 1, blk%ibNodes
            WRITE(file_unit,*) blk%xnode1(inode),blk%ynode1(inode),blk%znode1(inode), 99
          END DO
          DO ielem = 1, blk%ibElems
            WRITE(file_unit,*) blk%ibElP1(ielem),blk%ibElP2(ielem),blk%ibElP3(ielem)
          END DO
          CLOSE(file_unit)
